...
export function Methodology() {
  return (
    <section
      id="metodologia"
      className="relative py-28 md:py-36 surface-deep text-on-deep overflow-hidden"
    >
      <div
      ...
      <div className="container-x relative grid lg:grid-cols-2 gap-16 items-center">
        <div>
          ...
          <h2 className="mt-4 text-4xl md:text-5xl font-semibold text-on-deep text-balance">
            Um método que une <span className="text-gradient-accent">ciência, norma e prática</span>{" "}
            industrial.
          </h2>
          <p className="mt-5 text-lg text-muted-on-deep leading-relaxed max-w-xl">
            Cada projeto segue um protocolo claro de investigação técnica — para que o resultado
            entregue não seja apenas correto, mas defensável diante de qualquer instância.
          </p>
          ...
          <ol className="mt-12 space-y-8">
            {steps.map((s) => (
              <li
                key={s.n}
                className="grid grid-cols-[auto_1fr] gap-6 border-l hairline-border pl-6 -ml-6"
              >
                <span className="text-3xl font-semibold text-accent font-mono">{s.n}</span>
                <div>
        ...
        <div className="relative">
          ...
          <div className="absolute -bottom-8 -left-8 hidden md:block rounded-xl bg-card text-card-foreground p-6 shadow-elegant max-w-xs">
            ...
            <p className="mt-2 text-sm leading-relaxed text-foreground/80">
              &ldquo;Transformamos conhecimento técnico em decisões seguras para nossos
              clientes.&rdquo;
            </p>
          </div>
        </div>
